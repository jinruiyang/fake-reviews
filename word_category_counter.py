...
# Please be careful when editing the code, it is more complicated than
# would be ideal
...
# This actually captures most of what LIWC does...., I have no idea why it
# sells...
_liwc_tokenizer = re.compile(
    r'(\d[\d\.\,\-\:]*\d\.?|[a-zA-Z][a-zA-Z\.\']*[a-zA-Z]|\S|\n)', re.UNICODE | re.IGNORECASE)
...
def score_file(filename, raw_counts=False, scores=None, unique_words=None):
  return score_text(open(filename, encoding="latin1").read(), raw_counts=raw_counts, scores=scores, unique_words=unique_words)
...
class Dictionary():
  ...
  def __init__(self, filename, use_long_category_names=True, internal_category_list=None):
    ...
    # caches words that are searched for but not found, this favors processing
    # over memory
    self._ignored = set()

    self._setup_category_lookup(
        internal_category_list, use_long_category_names)
    try:
  ...
  _dictionary_line_categories_re = re.compile(
      r'(\d+|\<(\w+(\s+\w+)*)\>(\d+)(\/(\d+))?|\(\s*(\d+(\s+\d+)*)\s*\)(\d+)(\/(\d+))?)')
  ...
  def load_dictionary_file(self, filename, internal_category_list=None):
    ...
    for line in open(filename):
      ...
      word, is_stem, all_word_categories = Dictionary._dictionary_line_re.match(
          line).groups()
      for category_group in Dictionary._dictionary_line_categories_re.findall(all_word_categories):
        ...
        if category == '00':
          ...
          if is_stem == '*':
            ...
            self._lookup[word][Dictionary._TYPE_BASIC].add(
                self._category_lookup[int(category)])

        # convoluted special cases lead to much of the complexity in this
        # program
          ...
          if pre != '':
            ...
            conditions = sorted([self._category_lookup[int(number)]
                                 for number in pre.split()])
          ...
          for other_conditions, other_if_set, other_if_not_set in self._lookup[word][entry_type]:
            # a little costly on load means less on use
            if str(other_conditions) == str(conditions):
              other_if_set.add(if_true)
              other_if_not_set.add(if_not_true)
              break
          else:  # for else means the for ended naturally
            self._lookup[word][entry_type].append(
                (conditions, {if_true}, {if_not_true}))
  ...
  _pure_punctuation_re = re.compile(
          ...
          for char in word:
            if char in current_node:
              ...
              if isinstance(current_node, set):
                if Dictionary._TYPE_BASIC not in self._lookup[word]:
                  self._lookup[word][Dictionary._TYPE_BASIC] = set()
                self._lookup[word][Dictionary._TYPE_BASIC].update(
                    current_node)  # add to main lookup for time efficiency
      ...
      for long_name, LIWC2007_number, LIWC2007_short, LIWC2001_number, LIWC2001_short in Dictionary._liwc_categories:
        if internal_category_list == '2001' and LIWC2001_number is not None:
          self._category_lookup[
              LIWC2001_number] = self._translate_category_name(LIWC2001_short)
        if internal_category_list == '2007' and LIWC2007_number is not None:
          self._category_lookup[
              LIWC2007_number] = self._translate_category_name(LIWC2007_short)
...
_dictionary_filename = os.path.abspath(os.path.join(
    os.path.dirname(__file__), 'data')) + '/liwc/LIWC2007.dic'
